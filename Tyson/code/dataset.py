...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print('image.shape in IMtoIM2d (final return)', image.shape)
        # print('mask.shape in IMtoIM2d (final return)', mask.shape)        
...
class Image2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(os.path.join(self.input_path, image_filename))
        # image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)        
        # print('image.shape in IM2d, before correction', image.shape)

        # correct dimensions if needed
        image = correct_dims(image)

        image = self.transform(image)
        # print('image.shape in IM2d, after correction & transform', image.shape)
